...
const register = (req, res) => {
    ...
    pool.getConnection((err, connection) => {
        ...
        connection.query(checkEmailQuery, [data.email], (err, results) => {
            ...
            if (results.length > 0) {
                responseFailValidate(res, 'Email sudah digunakan. Gunakan email lain.');
            } else {
                ...
                connection.query(insertUserQuery, [data], (err, results) => {
                    ...
                    if (results.affectedRows >= 1) {
                        ...
                        res.status(500).json({ success: false, message: 'Failed creating user' });
                    }
                });
            }
        ...
        pool.getConnection((err, connection ) => {
            ...
            connection.query(query, (err, results) => {
                ...
                if (results.length >= 1){
                    ...
                    responAuthSucces(res, token, 'Login Success', {
                        email: user.email,
                        username: user.username
                    })

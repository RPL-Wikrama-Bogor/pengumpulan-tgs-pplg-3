...
<body>
    ...
    class Beli extends Shell {
        ...
        public function total() {
            $sebelum_ppn = $this->harga * $this->jumlah;
            $setelah_ppn = $sebelum_ppn * (1 + $this->ppn);
            return [$sebelum_ppn, $setelah_ppn];
        }

        public function tampilkan_bukti() {
            list($sebelum_ppn, $setelah_ppn) = $this->total();
            echo "Dengan jumlah: {$this->jumlah} Liter<br>";
            echo "Anda membeli bahan bakar minyak tipe {$this->tipe}<br>";
            echo "Total sebelum PPN: Rp. " . number_format($sebelum_ppn, 2) . "<br>";
            echo "Total yang harus anda bayar Rp. " . number_format($setelah_ppn, 2) . "<br>";
        }
    }

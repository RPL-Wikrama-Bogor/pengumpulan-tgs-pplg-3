...
@push('script')
    <script>
        // mendefinisikan no sebagai 2
        let no = 2;
        //ketika tag dengan id add-select di click jelankan funs berikut 
        $("#add-select").on("click", function() {
            // tag html yang akan ditambahkan/dimunculkan
            let el = `<b><select name="medicines[]" id="medicines" class="form-select">
            <option selected hidden disabled>Pesanan ${no}</option>
            @foreach ($medicines as $item)
            <option value="{{ $item['id'] }}">{{ $item['name'] }}</option>
            @endforeach
            </select>`;
            // apped : tambahkan elemen html di bagian seelum penutup tag penutup terkait (sebelum penutup tag yang id nya wrap-medicines)
